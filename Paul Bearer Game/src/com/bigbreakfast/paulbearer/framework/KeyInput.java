...
import com.bigbreakfast.paulbearer.window.Menu;
...
public class KeyInput extends KeyAdapter {
	...
	public void keyPressed(KeyEvent e){
		...
		if (Game.getState() == State.MENU) {
			...
			if (key == KeyEvent.VK_S) menu.selected++;
			if (key == KeyEvent.VK_W) menu.selected--; 
			if (menu.selected == menu.getMenuOptions().size()) menu.selected = 0;
			else if (menu.selected < 0) menu.selected = (menu.getMenuOptions().size() - 1);
	...
	//				playerY = tempObject.getY();	
					...
					if(key == KeyEvent.VK_E && player.isColliding())
					{
						...
						handler.addObject(new TextBox(playerX, playerY, "DialogBox", handler.getInventory().getInventoryItems(), ObjectId.TextBox));
